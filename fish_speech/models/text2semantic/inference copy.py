...
def decode_one_token_ar(
    ...
    **sampling_kwargs,
    ...
    codebooks = torch.stack(codebooks, dim=0).clone()  # <-- clone the stacked tensor to break CUDA Graphs reference
    # semantic_ids_tensor = torch.tensor(semantic_ids, device=codebooks.device)
    # codebooks[1:, :] = torch.masked_fill(
...
def load_model(checkpoint_path, device, precision, compile=False, is_agent=False):
    ...
    # Pre-create fixed parameter tensors to avoid runtime creation which can trigger
    # repeated torch.compile graph builds and GPU memory leaks.
    try:
        ...
        # In case device/precision isn't available yet, fall back to CPU tensors.
        model.fixed_temperature = torch.tensor(0.7, dtype=torch.float)
        model.fixed_top_p = torch.tensor(0.7, dtype=torch.float)
        model.fixed_repetition_penalty = torch.tensor(1.5, dtype=torch.float)
...
def generate_long(
    *,
    ...
    # Use pre-created tensors on the model to avoid creating new tensors repeatedly,
    # which can cause torch.compile to re-build graphs and leak GPU memory.
    try:
        temp_tensor = getattr(model, "fixed_temperature")
        top_p_tensor = getattr(model, "fixed_top_p")
        rep_tensor = getattr(model, "fixed_repetition_penalty")
    except Exception:
        temp_tensor = torch.tensor(temperature, device=device, dtype=torch.float)
        top_p_tensor = torch.tensor(top_p, device=device, dtype=torch.float)
    ...
    if abs(temp_tensor.item() - float(temp_val)) > 1e-6:
        temp_tensor.fill_(float(temp_val))
    if abs(top_p_tensor.item() - float(top_p_val)) > 1e-6:
        top_p_tensor.fill_(float(top_p_val))
    if abs(rep_tensor.item() - float(rep_val)) > 1e-6:
        rep_tensor.fill_(float(rep_val))
    ...
    for sample_idx in range(num_samples):
        ...
        while seg_idx < len(encoded):
            ...
            lengths = list(reversed([seg.size(1) for seg in global_encoded]))
...
def main(
    ...
    if prompt_text is not None and prompt_tokens is not None and len(prompt_text) != len(
        prompt_tokens
    ):
        raise ValueError(
    ...
    if prompt_tokens is not None:
        prompt_tokens_list = [torch.from_numpy(np.load(p)).to(device) for p in prompt_tokens]
